...
namespace MorningBird.Sound
{
    public class BasicSoundClipPlay_Common : MonoBehaviour
    {
        ...
        [SerializeField] bool _isSoundFollowingObject;
        [SerializeField] ObjectCondition _followingObjectCondition;
        ...
        private void FixedUpdate()
        {
            ...
            if(_isSoundFollowingObject == true)
            {
                CheckFollowingObjectCondition();
                DoActionFollowingObjectAmbience();
            }
        }

        private void CheckFollowingObjectCondition()
        {
            if(_followObjectTransform == null)
            {
                _followingObjectCondition = ObjectCondition.Destroy;
            }
            else if(_followObjectTransform.gameObject.activeInHierarchy == true)
            {
                _followingObjectCondition = ObjectCondition.Alive;
            }
            else if(_followObjectTransform.gameObject.activeInHierarchy == false)
            {
                _followingObjectCondition = ObjectCondition.Disable;
            }
            else
            {
                _followingObjectCondition = ObjectCondition.None;
            }

        }

        private void DoActionFollowingObjectAmbience()
        {
            switch (_followingObjectCondition)
            {
                case ObjectCondition.Alive:
                    break;
                case ObjectCondition.None:
                case ObjectCondition.Disable:
                case ObjectCondition.Destroy:
                    ReturnToPool();
                    break;
                default:
                    ReturnToPool();
                    Debug.Assert(false, "Undefined input");
                    break;
            }
        }
